...
For the EK-RA8P1 the payload must be the *Vela-compiled* model (target
ethos-u55-256) so the ethos-u custom op carries the NPU command stream.

Outputs, next to the input (or under --out-dir):
  <name>_v<ver>.iotv   the enveloped model
...
IOTV_HDR_LEN = 32
NAME_LEN = 16
# Must match MODEL_STORE_SLOT_SIZE in src/model_store/model_store.h
MODEL_MAX = 8 * 1024 * 1024 - IOTV_HDR_LEN
...
def main() -> None:
    ...
    ap.add_argument("tflite", type=pathlib.Path,
                    help="input Vela-compiled .tflite model")
