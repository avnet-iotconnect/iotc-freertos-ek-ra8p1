...
static inline void td_raw_putc( char c )
{
...
static void td_raw_puts( const char *s ) { while( *s ) td_raw_putc( *s++ ); }
static void td_raw_hex32( uint32_t v )
{
    static const char hex[] = "0123456789ABCDEF";
    for( int i = 7; i >= 0; i-- ) td_raw_putc( hex[ ( v >> ( i * 4 ) ) & 0xF ] );
}
                                           ...
                                           const size_t bufLen )
{
    ...
    if( retStatus == 0 )
    {
        sslRet = mbedtls_sha256( pCert->raw.p,
                                 pCert->raw.len,
                                 fingerprint,
                                 0 );
        if( sslRet != 0 )
...
#else /* #if ( MBEDTLS_VERSION_NUMBER >= 0x03000000 ) */
                            ...
                            if( mbedtlsRet == 0 )
                            {
                                              ...
                                              gmtime( &timeT ) ) != MBEDTLS_X509_RFC5280_UTC_TIME_LEN )
                                {
                                                  ...
                                                  gmtime( &timeT ) ) != MBEDTLS_X509_RFC5280_UTC_TIME_LEN )
                                    {
                                        ...
                                        /* mbedTLS 3.6: MPI-based set_serial is no longer declared;
                                         * use the raw-bytes variant with the same random serial. */
                                        if( mbedtls_x509write_crt_set_serial_raw( pWriteCert,
                                                                                  certSn,
                                                                                  sizeof( certSn ) ) == 0 )
                                        {
                                          ...
                                          size_t * pReadBufferSize )
{
    ...
    if( returnStatus == DTLS_SUCCESS )
    {
        ...
        while( mbedtlsError == MBEDTLS_ERR_SSL_WANT_READ && pDtlsTransportParams->pReceivedPacket != NULL )
        {
            ...
            td_raw_puts( "[mbT] r<" ); td_raw_hex32( ( uint32_t ) mbedtlsError ); td_raw_puts( "\r\n" );
    ...
    if( returnStatus == DTLS_SUCCESS )
    {
        ...
        *pReadBufferSize = ( size_t ) readOffset;
        ...
        if( pNetworkContext->state == DTLS_STATE_HANDSHAKING )
        {
            ...
            if( pDtlsTransportParams->dtlsSslContext.context.MBEDTLS_PRIVATE(state) == MBEDTLS_SSL_HANDSHAKE_OVER )
            {
