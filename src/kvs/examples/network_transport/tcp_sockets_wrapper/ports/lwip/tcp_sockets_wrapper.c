 ...
 */
BaseType_t KvsTCP_Sockets_Connect( Socket_t * pTcpSocket,
                                ...
                                uint32_t sendTimeoutMs )
{
    ...
    {
        *pTcpSocket = pvPortMalloc( sizeof( struct xKVS_SOCKET ) );
        if( *pTcpSocket == NULL )
        {
 ...
 */
void KvsTCP_Sockets_Disconnect( Socket_t tcpSocket )
{
    if( tcpSocket == NULL )
    {
        return;
 ...
 *
 * The socket must have already been created using a call to KvsTCP_Sockets_Connect().
 *
 ...
 */
int32_t KvsTCP_Sockets_Send( Socket_t xSocket,
                          const void * pvBuffer,
 ...
 * * If an error occurred, a negative value is returned. @ref SocketsErrors
 */
int32_t KvsTCP_Sockets_Recv( Socket_t xSocket,
                          void * pvBuffer,
                          size_t xBufferLength )
{
    int xReadRet;
...
int32_t KvsTCP_Sockets_GetSocketFd( Socket_t xSocket )
{
    int32_t ret = -1;
...
BaseType_t KvsTCP_Sockets_ConfigureTimeout( Socket_t xSocket,
                                         uint32_t receiveTimeoutMs,
                                         uint32_t sendTimeoutMs )
{
    BaseType_t xRet = TCP_SOCKETS_ERRNO_NONE;
