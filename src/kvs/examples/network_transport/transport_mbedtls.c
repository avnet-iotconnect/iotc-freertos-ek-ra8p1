...
/* Watchdog feeder — the TLS write path is long-running and must pet IWDG. */
                                          ...
                                          uint32_t flags )
{
    ...
    if( returnStatus == TLS_TRANSPORT_SUCCESS )
    {
        ...
        if( ( flags & TLS_CONNECT_NON_BLOCKING_HANDSHAKE ) == 0 )
        {
            do
            {
                returnStatus = KVSTLS_ContinueHandshake( pTlsNetworkContext );
...
TlsTransportStatus_t KVSTLS_ContinueHandshake( TlsNetworkContext_t * pTlsNetworkContext )
{
    TlsTransportParams_t * pTlsTransportParams = NULL;
    TlsTransportStatus_t returnStatus = TLS_TRANSPORT_SUCCESS;
    int32_t mbedtlsError = 0;
...
TlsTransportStatus_t KVSTLS_Connect( TlsNetworkContext_t * pTlsNetworkContext,
                                           const char * pHostName,
                                           uint16_t port,
                                           const NetworkCredentials_t * pNetworkCredentials,
                                           ...
                                           uint32_t flags )
{
    ...
    if( returnStatus == TLS_TRANSPORT_SUCCESS )
    {
        ...
        socketStatus = KvsTCP_Sockets_Connect( &( pTlsTransportParams->tcpSocket ),
                                            pHostName,
                                            port,
                                            receiveTimeoutMs,
                                            sendTimeoutMs );
        ...
        ( returnStatus != TLS_TRANSPORT_HANDSHAKE_IN_PROGRESS ) )
    {
        ...
        {
            KvsTCP_Sockets_Disconnect( pTlsTransportParams->tcpSocket );
            pTlsTransportParams->tcpSocket = NULL;
        }
    }
    else
...
TlsTransportStatus_t KVSTLS_Disconnect( TlsNetworkContext_t * pTlsNetworkContext )
{
    TlsTransportParams_t * pTlsTransportParams = NULL;
...
int32_t KVSTLS_recv( TlsNetworkContext_t * pTlsNetworkContext,
                           void * pBuffer,
                           size_t bytesToRecv )
{
    TlsTransportParams_t * pTlsTransportParams = NULL;
...
int32_t KVSTLS_send( TlsNetworkContext_t * pTlsNetworkContext,
                           const void * pBuffer,
                           size_t bytesToSend )
{
    TlsTransportParams_t * pTlsTransportParams = NULL;
    ...
    else
    {
        ...
        /* Pet the watchdog bracketing the TLS write so a slow mbedTLS write
         * cannot trip the IWDG. */
        vPetWatchdog();
...
int32_t KVSTLS_GetSocketFd( TlsNetworkContext_t * pTlsNetworkContext )
{
    int32_t ret = -1;

    if( ( pTlsNetworkContext != NULL ) && ( pTlsNetworkContext->pParams != NULL ) )
    {
        ret = KvsTCP_Sockets_GetSocketFd( pTlsNetworkContext->pParams->tcpSocket );
    }
...
TlsTransportStatus_t KVSTLS_ConfigureTimeout( TlsNetworkContext_t * pTlsNetworkContext,
                                                    uint32_t receiveTimeoutMs,
                                                    uint32_t sendTimeoutMs )
{
    TlsTransportStatus_t returnStatus = TLS_TRANSPORT_SUCCESS;
    ...
    if( returnStatus == TLS_TRANSPORT_SUCCESS )
    {
        tcpResult = KvsTCP_Sockets_ConfigureTimeout( pTlsNetworkContext->pParams->tcpSocket, receiveTimeoutMs, sendTimeoutMs );
        if( tcpResult != TCP_SOCKETS_ERRNO_NONE )
        {
            LogError( ( "Failed to configure timeout: KvsTCP_Sockets_ConfigureTimeout returned error %ld.",
                        tcpResult ) );
            returnStatus = TLS_TRANSPORT_INTERNAL_ERROR;
        }
    }
