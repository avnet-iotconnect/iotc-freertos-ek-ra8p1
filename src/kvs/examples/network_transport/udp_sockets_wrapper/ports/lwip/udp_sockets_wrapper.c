...
#include "kvs_udp_sockets_wrapper.h"
...
BaseType_t KvsUDP_Sockets_CreateAndAssign( Socket_t * pUdpSocket,
                                        int assignFd )
{
    ...
    {
        *pUdpSocket = pvPortMalloc( sizeof( struct xKVS_SOCKET ) );
        if( *pUdpSocket == NULL )
        {
 ...
 */
BaseType_t KvsUDP_Sockets_Connect( Socket_t * pUdpSocket,
 ...
 */
void KvsUDP_Sockets_Disconnect( Socket_t udpSocket )
{
 ...
 *
 * The socket must have already been created using a call to KvsUDP_Sockets_Connect().
 *
 ...
 */
int32_t KvsUDP_Sockets_Send( Socket_t xSocket,
                          const void * pvBuffer,
                          size_t xBufferLength )
{
    ...
    xWriteRet = send( xSocket->xFd, pvBuffer, xBufferLength, 0 );
    if( xWriteRet >= 0 )
    {
 ...
 * * If an error occurred, a negative value is returned. @ref SocketsErrors
 */
int32_t KvsUDP_Sockets_Recv( Socket_t xSocket,
                          void * pvBuffer,
                          size_t xBufferLength )
{
    int xReadRet;
    ...
    xReadRet = recv( xSocket->xFd, pvBuffer, xBufferLength, 0 );
    if( xReadRet >= 0 )
    {
...
int32_t KvsUDP_Sockets_GetSocketFd( Socket_t xSocket )
{
    int32_t ret = -1;
