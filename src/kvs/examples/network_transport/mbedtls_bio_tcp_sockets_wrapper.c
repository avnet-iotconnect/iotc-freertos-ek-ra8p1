...
/* Watchdog feeder — the BIO TCP send is called under TLS and can block,
 * so pet IWDG around it to prevent a slow network from tripping the watchdog. */
                                      ...
                                      size_t len )
{
    ...
    /* Pet the watchdog around the blocking TCP send so a slow network
     * cannot trip the IWDG. */
    vPetWatchdog();
    xReturnStatus = KvsTCP_Sockets_Send( ( Socket_t ) ctx, buf, len );
    vPetWatchdog();
                                      ...
                                      size_t len )
{
    ...
    xReturnStatus = KvsTCP_Sockets_Recv( ( Socket_t ) ctx, buf, len );
