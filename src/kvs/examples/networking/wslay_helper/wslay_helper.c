...
static int32_t SendTlsPacket( NetworkContext_t * pNetworkContext,
                              ...
                              size_t bytesToSend )
{
    return KVSTLS_send( ( TlsNetworkContext_t * ) pNetworkContext, pBuffer, bytesToSend );
}
...
static int32_t RecvTlsPacket( NetworkContext_t * pNetworkContext,
                              ...
                              size_t bytesToRecv )
{
    return KVSTLS_recv( ( TlsNetworkContext_t * ) pNetworkContext, pBuffer, bytesToRecv );
}
                                     ...
                                     WebsocketServerInfo_t * pServerInfo )
{
    ...
    if( ret == NETWORKING_WSLAY_RESULT_OK )
    {
        do
        {
            ...
            /* Try connect with websocket server. */
            {
                ...
                xNetworkStatus = KVSTLS_Connect( &pWebsocketCtx->xTlsNetworkContext,
                                                       host,
                                                       443,
                                                       &credentials,
            ...
            xNetworkStatus = KVSTLS_ConfigureTimeout( &pWebsocketCtx->xTlsNetworkContext, 0U, 0U );
            if( xNetworkStatus != TLS_TRANSPORT_SUCCESS )
            {
    ...
    {
        ( void ) KVSTLS_Disconnect( &pWebsocketCtx->xTlsNetworkContext );
    }
...
WebsocketResult_t Websocket_Disconnect( NetworkingWslayContext_t * pWebsocketCtx )
{
    ...
    if( ret == WEBSOCKET_RESULT_OK )
    {
        ...
        if( pWebsocketCtx->connectionEstablished != 0U )
        {
            tlsStatus = KVSTLS_Disconnect( &pWebsocketCtx->xTlsNetworkContext );
...
WebsocketResult_t Websocket_Recv( NetworkingWslayContext_t * pWebsocketCtx )
{
    ...
    fd = KVSTLS_GetSocketFd( &pWebsocketCtx->xTlsNetworkContext );
