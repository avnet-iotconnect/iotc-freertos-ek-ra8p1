 ...
 * The `logging.h` resolved for this TU is the AWS-KVS one whose SdkLog() is
 * plain printf, which is NOT wired to the board UART on this project — so
 * LogInfo() calls from app_common.c are invisible.  Use these helpers
 * (same pattern as icn_raw_* / venc_raw_*) to emit `[ac]` traces for the
 * second-viewer black-screen investigation.                                */
extern void vPetWatchdog( void );
static inline void ac_raw_putc( char c )
{
...
static void ac_raw_puts( const char *s ) { while( *s ) ac_raw_putc( *s++ ); }
static void ac_raw_putn( const char *s, int n ) { while( n-- > 0 ) ac_raw_putc( *s++ ); }
static void ac_raw_dec( unsigned int v )
{
    char buf[ 12 ];
    int  n = 0;
    if( v == 0 ) { ac_raw_putc( '0' ); return; }
    while( v > 0 ) { buf[ n++ ] = '0' + ( v % 10 ); v /= 10; }
    while( n-- ) ac_raw_putc( buf[ n ] );
}
static void ac_raw_hex( unsigned int v )
{
                                   ...
                                   const SignalingMessage_t * pSignalingMessage )
{
    ...
    /* Second-viewer black-screen diagnostic: confirms remote candidate arrived
     * on the wss channel and shows which AppSession slot it dispatched to.
     * slot=-1 means no slot was available.  If viewer 2 is silently stuck
     * with no pairs, look for `[ac] cand_rx` in the log.  Raw UART bypass —
     * the AWS-KVS logging.h picked up by this TU isn't wired to the board
     * UART, so LogInfo() here is silent.                                    */
    ac_raw_puts( "[ac] cand_rx cid=" );
    ac_raw_dec( ( unsigned int ) pSignalingMessage->remoteClientIdLength );
    ac_raw_putc( '/' );
    ac_raw_putn( pSignalingMessage->pRemoteClientId,
                 ( int ) pSignalingMessage->remoteClientIdLength );
    ac_raw_puts( " slot=" );
    if( pAppSession != NULL )
    {
        ac_raw_dec( ( unsigned int )( pAppSession - pAppContext->appSessions ) );
    }
    else
    {
        ac_raw_puts( "-1" );
    ...
    ac_raw_puts( " msg=" );
    ac_raw_dec( ( unsigned int ) pSignalingMessage->messageLength );
    ac_raw_puts( "\r\n" );
                                       ...
                                       void * pUserData )
{
    ...
    /* Second-viewer black-screen diagnostic: one-line summary of every wss
     * signaling message reaching the device callback, including type and
     * sender clientId.  Pairs with the `[ac] cand_rx` line in
     * HandleRemoteCandidate — together they show whether viewer 2's ICE
     * candidates (a) never arrive here, (b) arrive as some other type,
     * or (c) arrive and are dispatched.  Raw UART bypass — LogInfo() from
     * this TU doesn't reach the board UART.                                 */
    ac_raw_puts( "[ac] wss_msg type=" );
    ac_raw_hex( ( unsigned int ) pSignalingMessage->messageType );
    ac_raw_puts( " cid=" );
    ac_raw_dec( ( unsigned int ) pSignalingMessage->remoteClientIdLength );
    ac_raw_putc( '/' );
    ac_raw_putn( pSignalingMessage->pRemoteClientId,
