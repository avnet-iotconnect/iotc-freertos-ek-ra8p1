...
static void SessionProcessEndlessLoop( PeerConnectionSession_t * pSession )
{
    ...
    for( ; !skipProcess; )
    {
        ...
        /* Pet the watchdog after each message — ICE connectivity checks
         * can keep this task busy for tens of seconds (20+ candidate pairs
         * × 200 ms timer), starving the idle hook that normally feeds it. */
        vPetWatchdog();
                                                          ...
                                                          size_t decodeMessageLength )
{
    ...
    if( ret == PEER_CONNECTION_RESULT_OK )
    {
        ...
        if( iceControllerResult != ICE_CONTROLLER_RESULT_OK )
        {
            /* result 52 (JSON_CANDIDATE_LACK_OF_ELEMENT) is typically an
             * empty or end-of-candidates trickle message — harmless. */
            if( iceControllerResult == ICE_CONTROLLER_RESULT_JSON_CANDIDATE_LACK_OF_ELEMENT )
            {
                LogDebug( ( "Empty/end-of-candidates trickle message (result: %d), ignoring.", iceControllerResult ) );
            }
            else
            {
