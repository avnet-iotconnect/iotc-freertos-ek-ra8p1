                                                                ...
                                                                const PeerConnectionFrame_t * pFrame )
{
    ...
    if( ret == PEER_CONNECTION_RESULT_OK )
    {
        ...
        /* Bounded 500 ms wait — see rationale above senderMutex give.
         * If the previous frame's send wedged (deep in SRTP encrypt, TURN
         * channel-data framing, or socket write), we must not wait forever:
         * silently accumulating blocked media tasks fills the FreeRTOS
         * scheduler and starves IWDG feeding.  On timeout, log ONCE and
         * return — the frame is dropped and the caller reports the failure. */
    ...
    {
        vPetWatchdog();
        /* Per-packet traces removed — at 115200 baud the ~410 chars/packet
         * of debug output took ~36ms each, far exceeding the 66ms frame
         * period and causing the media task to fall behind the camera.       */
        ...
        if( ret == PEER_CONNECTION_RESULT_OK )
        {
            ...
            /* Update the packet into rolling buffer. */
