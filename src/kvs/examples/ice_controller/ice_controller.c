                                             ...
                                             uint16_t * pOutputBufferLength )
{
    ...
    {
        retMbedtls = mbedtls_md5( pBuffer, bufferLength, pOutputBuffer );
        if( retMbedtls != 0 )
        {
    ...
    if( xSemaphoreTake( pCtx->iceMutex, portMAX_DELAY ) == pdTRUE )
    {
        for( i = 0; i < pCtx->socketsContextsCount; i++ )
        {
            ...
            else if( pSocketContext->pLocalCandidate != NULL )
            {
                ...
                /* Release iceMutex before send + delay so the socket listener
                 * can process incoming TURN responses in the meantime. */
                ...
                if( xSemaphoreTake( pCtx->iceMutex, portMAX_DELAY ) != pdTRUE )
                {
                    LogError( ( "Failed to re-acquire iceMutex in ProcessLocalCandidates" ) );
                    return;
                }
            }
        }
                                                         ...
                                                         IceCandidatePair_t * pTargetCandidatePair )
{
    ...
    /* Visible diagnostic for relay pair state machine (CREATE_PERMISSION / CHANNEL_BIND).
     * Log the transaction ID so we can match it against the RX side. */
    if( ( pTargetCandidatePair->state == ICE_CANDIDATE_PAIR_STATE_CREATE_PERMISSION ) ||
        ( pTargetCandidatePair->state == ICE_CANDIDATE_PAIR_STATE_CHANNEL_BIND ) )
    {
        const uint8_t * tid = pTargetCandidatePair->transactionId;
        LogInfo( ( "RELAY TX pair 0x%04x/0x%04x st=%d txn=%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
                   pTargetCandidatePair->pLocalCandidate->candidateId,
                   pTargetCandidatePair->pRemoteCandidate->candidateId,
                   pTargetCandidatePair->state,
...
static void ProcessCandidatePairs( IceControllerContext_t * pCtx )
{
    ...
    if( result == ICE_CONTROLLER_RESULT_OK )
    {
        /* Round-robin: start where we left off last tick. */
        static uint32_t nextPairIdx = 0;
        if( nextPairIdx >= count )
        {
            nextPairIdx = 0;
        }

        for( checked = 0; checked < ICE_CONTROLLER_MAX_PAIRS_PER_TICK && checked < count; checked++ )
        {
            i = ( nextPairIdx + checked ) % count;
            ...
            /* Release iceMutex during the delay so the socket listener can
             * process incoming STUN responses (e.g. CreatePermission).  Without
             * this, the mutex is held for the entire loop (~150 ms), blocking
             * the receive path from transitioning pair states. */
            xSemaphoreGive( pCtx->iceMutex );
            vTaskDelay( pdMS_TO_TICKS( 50 ) );
            if( xSemaphoreTake( pCtx->iceMutex, portMAX_DELAY ) != pdTRUE )
            {
                LogError( ( "Failed to re-acquire iceMutex after delay" ) );
                isLocked = 0U;
                result = ICE_CONTROLLER_RESULT_FAIL_MUTEX_TAKE;
                break;
            }
        ...
        nextPairIdx = ( nextPairIdx + checked ) % ( count > 0 ? count : 1 );
    }
...
IceControllerResult_t IceController_ProcessIceCandidatesAndPairs( IceControllerContext_t * pCtx )
{
    ...
    if( ret == ICE_CONTROLLER_RESULT_OK )
    {
        ...
        /* Pet watchdog after local candidate processing. */
        vPetWatchdog();
    }

    if( ret == ICE_CONTROLLER_RESULT_OK )
    {
        /* Re-read current time — processing may have taken seconds. */
        currentTimeMs = NetworkingUtils_GetCurrentTimeUs( NULL ) / 1000;
    ...
    if( ret == ICE_CONTROLLER_RESULT_OK )
    {
        if( pCtx->pNominatedSocketContext->pLocalCandidate->candidateType == ICE_CANDIDATE_TYPE_RELAY )
        {
            ...
            else
            {
                ...
                if( xSemaphoreTake( pCtx->iceMutex, pdMS_TO_TICKS( 500 ) ) == pdTRUE )
                {
