 ...
 * leaves plenty of margin for mbedTLS / DTLS / lwIP.                         */
#define DEFAULT_TRANSCEIVER_ROLLING_BUFFER_DURACTION_SECOND ( 1 )

/* TRANSCEIVER_H264_VIDEO_BIT_RATE drives the RTP rolling-buffer size:
 *   capacity = duration × bitRate / 8 / maxSizePerPacket.
 *
 * At 500 kbps / 1 s → 53 packets × ~1.4 KB = ~74 KB per viewer.  With two
 * concurrent viewers that's ~148 KB of rolling-buffer packet data alone,
 * leaving only ~61 KB free heap — too tight for I-frame bursts + TURN TX
 * buffers, causing the second viewer's session to die within seconds.
 *
 * Halving to 250 kbps → ~26 packets × 1.4 KB ≈ 37 KB per viewer, saving
 * ~74 KB with two viewers.  The encoder's actual sustained output at the
 ...
 * actual packet buffers are never filled.  Lowered from 1 Mbps.
 * Minimum usable value: RtpPacketQueue_Init requires capacity >= 2, and
 * capacity = duration × bitRate / 8 / maxSizePerPacket.  At 1s duration
 * and ~1400 B/pkt, 32 kbps → capacity = 4096/1400 = 2.                      */
#define DEFAULT_TRANSCEIVER_AUDIO_BIT_RATE ( 32 * 1024 )
...
static int32_t OnFrameReadyToSend( void * pCtx,
                                   MediaFrame_t * pFrame );
static int32_t OnVideoFrameReadyToSend_Sync( void * pCtx,
                                             MediaFrame_t * pFrame );
...
static void VideoTx_Task( void * pParameter )
{
    ...
    if( skipProcess == 0 )
    {
        ...
        while( 1 )
        {
             ...
             * starve the idle hook that normally refreshes the IWDG. */
            vPetWatchdog();
        }
    }
    ...
    if( ret == 0 )
    {
                            ...
                            portMAX_DELAY ) == pdTRUE )
        {
            ...
            if( pMediaSource->pSourcesContext->totalNumReadyPeer == 1U )
            {
                 ...
                 * Video uses the synchronous variant so the producer
                 * blocks until SRTP packetization is fully done with
                 * the encoded buffer (avoids producer/consumer race on
                 * the single static ucEncodedFrameBuf).  Audio keeps
                 * the queue-based path.                                */
                ret = AppMediaSourcePort_Start( OnVideoFrameReadyToSend_Sync,
            ...
            LogInfo( ( "PeerReady tk=%d nReady=%u total=%u", pMediaSource->trackKind, pMediaSource->numReadyPeer, pMediaSource->pSourcesContext->totalNumReadyPeer ) );
        }
        else
        {
    ...
    if( ret == 0 )
    {
                            ...
                            portMAX_DELAY ) == pdTRUE )
        {
            ...
            if( pMediaSource->pSourcesContext->totalNumReadyPeer == 0U )
            {
                ...
                LogInfo( ( "PeerClosed total=0 => STOPPING MEDIA" ) );
                AppMediaSourcePort_Stop();
            }

            /* We have finished accessing the shared resource.  Release the mutex. */
            xSemaphoreGive( pMediaSource->pSourcesContext->mediaMutex );
            LogInfo( ( "PeerClosed tk=%d nReady=%u total=%u", pMediaSource->trackKind, pMediaSource->numReadyPeer, pMediaSource->pSourcesContext->totalNumReadyPeer ) );
        }
        else
        {
            LogError( ( "Failed to lock media mutex, track kind=%d.", pMediaSource->trackKind ) );
...
/* Synchronous variant for video.  The previous queue-based path passed
 * a MediaFrame_t struct (just pointers + size) to VideoTx_Task, which
 * then read from frame.pData asynchronously — meanwhile the producer
 * had already overwritten that single static encoded buffer with the
 * next frame.  Result: every RTP packet on the wire was a salad of
 * two different frames, and the receiver showed black video.
 *
 * Calling the sink hook synchronously here causes the producer
 * (prvMediaTask) to block until SRTP packetization of the previous
 * frame has fully completed, eliminating the race entirely.  Audio is
 * left on the queue path because audio frames are tiny and the audio
 * source may be capture-driven.                                       */
static int32_t OnVideoFrameReadyToSend_Sync( void * pCtx,
                                             MediaFrame_t * pFrame )
{
    AppMediaSourceContext_t * pMediaSource = ( AppMediaSourceContext_t * ) pCtx;
    ...
    if( pMediaSource->pSourcesContext->onMediaSinkHookFunc != NULL )
    {
        return pMediaSource->pSourcesContext->onMediaSinkHookFunc(
                   pMediaSource->pSourcesContext->pOnMediaSinkHookCustom,
                   pFrame );
    }
